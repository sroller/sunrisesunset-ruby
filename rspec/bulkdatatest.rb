...
describe SolarEventCalculator, "Test the sunset algorithm" do
  ...
  it "returns correct sunrise/sunset data over a year" do
    ...
    dataFile.readlines.each do |dataLine|
      ...
      date = Date.strptime(parts.shift, '%m/%d/%Y')
      ...
      expect(calc.compute_astronomical_sunrise(timeZone)).to be_close_to(expectedAstronomicalRise, "Astronomical Rise")

      time = parts[1].split(':')
      expectedNauticalRise = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_nautical_sunrise(timeZone)).to be_close_to(expectedNauticalRise, "Nautical Rise")

      time = parts[2].split(':')
      expectedCivilRise = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_civil_sunrise(timeZone)).to be_close_to(expectedCivilRise, "Civil Rise")

      time = parts[3].split(':')
      expectedOfficialRise = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_official_sunrise(timeZone)).to be_close_to(expectedOfficialRise, "Official Rise")

      time = parts[4].split(':')
      expectedOfficialSet = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_official_sunset(timeZone)).to be_close_to(expectedOfficialSet, "Official Set")

      time = parts[5].split(':')
      expectedCivilSet = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_civil_sunset(timeZone)).to be_close_to(expectedCivilSet, "Civil Set")

      time = parts[6].split(':')
      expectedNauticalSet = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_nautical_sunset(timeZone)).to be_close_to(expectedNauticalSet, "Nautical Set")

      time = parts[7].split(':')
      expectedAstronomicalSet = put_in_timezone(date, time[0], time[1], timeZone)
      expect(calc.compute_astronomical_sunset(timeZone)).to be_close_to(expectedAstronomicalSet, "Astronomical Set")
    end
    end
  end
end
...
RSpec::Matchers.define :be_close_to do |expected, type|
  ...
  failure_message do |actual|
    "Expected #{actual} to be close to #{expected}, but it wasn't withon +-60"
  end
end
...
Spec::Matchers.define :be_close_to do |expected, type|
  ...
  failure_message_for).to_be do |actual|
    " actual #{type} #{actual} is outside +-1 minute of expected  #{type} #{expected}"
  end

  failure_message_for).to_be_not do |actual|
